...
hyperparam_dict = {
    "learner_g": [("Linear", LinearRegression()),
                  ("LGBM", LGBMRegressor(n_estimators=300, learning_rate=0.05, num_leaves=10))],
...
df_results_detailed_qte = pd.DataFrame(
    columns=["Coverage", "CI Length",
             ...
             "Learner g", "Learner m",
             "level", "repetition"])
df_results_detailed_qte["Uniform Coverage"] = df_results_detailed_qte["Uniform Coverage"].astype(bool)
...
# save results
df_results_qte.to_csv("results/cvar_qte_coverage.csv", index=False)
df_results_pq0.to_csv("results/cvar_pq0_coverage.csv", index=False)
df_results_pq1.to_csv("results/cvar_pq1_coverage.csv", index=False)
