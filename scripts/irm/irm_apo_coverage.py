...
data_apo_large = make_irm_data_discrete_treatments(
    n_obs=int(1e6), n_levels=n_levels, linear=True
)
...
for i in range(n_rep):
    data_apo = make_irm_data_discrete_treatments(
        n_obs=n_obs, n_levels=n_levels, linear=True
    )
    df_apo = pd.DataFrame(
...
hyperparam_dict = {
    "learner_g": [("Linear", LinearRegression()), ("LGBM", LGBMRegressor(verbose=-1))],
    "learner_m": [
...
for i_rep in range(n_rep):
    ...
    for learner_g_idx, (learner_g_name, ml_g) in enumerate(
        ...
        for learner_m_idx, (learner_m_name, ml_m) in enumerate(
            ...
            for treatment_idx, treatment_level in enumerate(
                hyperparam_dict["treatment_levels"]
            ):
                ...
                for level_idx, level in enumerate(hyperparam_dict["level"]):
                    ...
                    coverage = (confint.iloc[0, 0] < apos[treatment_idx]) & (
                        apos[treatment_idx] < confint.iloc[0, 1]
                    )
            ...
            for level_idx, level in enumerate(hyperparam_dict["level"]):
                ...
                coverage = np.mean(
                    (confint.iloc[:, 0] < apos) & (apos < confint.iloc[:, 1])
                )
                ...
                coverage_uniform = all(
                    (confint_uniform.iloc[:, 0] < apos)
                    & (apos < confint_uniform.iloc[:, 1])
                )
                ...
                coverage_contrast = np.mean(
                    (confint_contrast.iloc[:, 0] < ates)
                    & (ates < confint_contrast.iloc[:, 1])
                )
                ci_length_contrast = np.mean(
                    confint_contrast.iloc[:, 1] - confint_contrast.iloc[:, 0]
                )

                causal_contrast_model.bootstrap(n_rep_boot=2000)
                confint_contrast_uniform = causal_contrast_model.confint(
                    level=level, joint=True
                )
                coverage_contrast_uniform = all(
                    (confint_contrast_uniform.iloc[:, 0] < ates)
                    & (ates < confint_contrast_uniform.iloc[:, 1])
                )
                ci_length_contrast_uniform = np.mean(
                    confint_contrast_uniform.iloc[:, 1]
                    - confint_contrast_uniform.iloc[:, 0]
                )
...
df_results_apo = (
    df_results_detailed_apo.groupby(
        ["Learner g", "Learner m", "Treatment Level", "level"]
    )
    .agg(
...
df_results_apos = (
    df_results_detailed_apos.groupby(["Learner g", "Learner m", "level"])
    .agg(
        {
            "Coverage": "mean",
            "CI Length": "mean",
...
df_results_apos_contrast = (
    df_results_detailed_apos_constrast.groupby(["Learner g", "Learner m", "level"])
    .agg(
        {
            "Coverage": "mean",
            "CI Length": "mean",
