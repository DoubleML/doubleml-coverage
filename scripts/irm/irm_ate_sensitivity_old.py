...
# test inputs
np.random.seed(42)
dgp_dict = make_confounded_irm_data(n_obs=int(1e6), **dgp_pars)
...
hyperparam_dict = {
    "learner_g": [
        ...
        (
            "LGBM",
            LGBMRegressor(
                n_estimators=500, learning_rate=0.01, min_child_samples=10, verbose=-1
            ),
        ),
    ],
    "learner_m": [
        ...
        for learner_m_idx, (learner_m_name, ml_m) in enumerate(
            ...
            for level_idx, level in enumerate(hyperparam_dict["level"]):
                ...
                dml_irm.sensitivity_analysis(
                    cf_y=cf_y, cf_d=cf_d, rho=rho, level=level, null_hypothesis=theta
                )
