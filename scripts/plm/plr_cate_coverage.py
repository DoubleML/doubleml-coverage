...
for i in range(n_rep):
    data = make_heterogeneous_data(
        n_obs=n_obs, p=p, support_size=support_size, n_x=n_x, binary_treatment=False
    )
    ...
    for learner_g_idx, (learner_g_name, ml_g) in enumerate(
        ...
        for learner_m_idx, (learner_m_name, ml_m) in enumerate(
            ...
            for level_idx, level in enumerate(hyperparam_dict["level"]):
                ...
                coverage = (confint.iloc[:, 0] < true_effects) & (
                    true_effects < confint.iloc[:, 2]
                )
                ci_length = confint.iloc[:, 2] - confint.iloc[:, 0]
                confint_uniform = cate.confint(
                    basis=spline_basis, level=0.95, joint=True, n_rep_boot=2000
                )
                coverage_uniform = all(
                    (confint_uniform.iloc[:, 0] < true_effects)
                    & (true_effects < confint_uniform.iloc[:, 2])
                )
                ci_length_uniform = (
                    confint_uniform.iloc[:, 2] - confint_uniform.iloc[:, 0]
                )
