...
# test inputs
dgp_dict = make_confounded_plr_data(n_obs=int(1e6), cf_y=cf_y, cf_d=cf_d)
...
hyperparam_dict = {
    ...
    "learner_g": [
        (
            "LGBM",
            LGBMRegressor(
                n_estimators=500, learning_rate=0.05, min_child_samples=5, verbose=-1
            ),
        ),
        (
            "Random Forest",
    ...
    "learner_m": [
        (
            "LGBM",
            LGBMRegressor(
                n_estimators=500, learning_rate=0.05, min_child_samples=2, verbose=-1
            ),
        ),
        (
            "Random Forest",
        ...
        for learner_g_idx, (learner_g_name, ml_g) in enumerate(
            ...
            for learner_m_idx, (learner_m_name, ml_m) in enumerate(
                ...
                for level_idx, level in enumerate(hyperparam_dict["level"]):
                    ...
                    bias_lower = abs(
                        theta - dml_plr.sensitivity_params["theta"]["lower"]
                    )
                    bias_upper = abs(
                        theta - dml_plr.sensitivity_params["theta"]["upper"]
                    )
...
df_results = (
    df_results_detailed.groupby(["Learner g", "Learner m", "score", "level"])
    .agg(
        {
            "Coverage": "mean",
            "CI Length": "mean",
