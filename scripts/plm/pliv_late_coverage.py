...
for i in range(n_rep):
    data = make_pliv_CHS2015(
        alpha=theta, n_obs=n_obs, dim_x=dim_x, dim_z=dim_z, return_type="DataFrame"
    )
    ...
    for score_idx, score in enumerate(hyperparam_dict["score"]):
        for learner_g_idx, (learner_g_name, ml_g) in enumerate(
            ...
            for learner_m_idx, (learner_m_name, ml_m) in enumerate(
                ...
                for learner_r_idx, (learner_r_name, ml_r) in enumerate(
                    hyperparam_dict["learner_r"]
                ):
                    if score == "IV-type":
...
df_results = (
    df_results_detailed.groupby(
        ["Learner g", "Learner m", "Learner r", "score", "level"]
    )
    .agg(
